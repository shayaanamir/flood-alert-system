...
const RiskAssessment = ({ weatherData }) => {
  ...
  const [riskDescription, setRiskDescription] = useState('Low flood risk. Continue to monitor conditions.');
  ...
  useEffect(() => {
    ...
    const humidity = weatherData.relative_humidity_2m || weatherData.humidity || 0;
    const windSpeed = weatherData.wind_speed_10m || weatherData.wind_speed || 0;
    
    let level = 'Low';
    if (precipitation > 10 || (humidity > 85 && windSpeed > 30)) {
      level = 'High';
    } else if (precipitation > 5 || humidity > 70) {
      level = 'Moderate';
    }
    setRiskLevel(level);
    ...
    if (level === 'High') {
      description = 'High flood risk. Take immediate precautions and stay alert.';
    } else if (level === 'Moderate') {
      description = 'Moderate flood risk. Be prepared to take action if conditions worsen.';
    }
    ...
    <div className={`risk-assessment-card risk-${riskLevel.toLowerCase()}`}>
      ...
      <div className="risk-header">
        <div className="risk-icon-circle">
          <svg xmlns="http://www.w3.org/2000/svg" height="36px" viewBox="0 -960 960 960" width="36px" fill="#EFEFEF">
