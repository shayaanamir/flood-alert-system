...
export const getAlertStats = async (req, res) => {
  try {
    // Count both Active and Pending alerts for overall and severity-specific stats
    const statusQuery = { status: { $in: ["Active", "Pending"] } };

    const total = await Alert.countDocuments(statusQuery);
